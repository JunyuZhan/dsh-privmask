...
- **诊断日志轮转**：`privmask-restore.log` 此前只追加不轮转，长期运行会无限增长；现在超过 5MB
  轮转为 `.1`（与离境审计 JSONL 同策略），写入串行化避免并发交错。
- **病理输入时间上限回归**：新增 6 个最坏形态输入（20 万连续汉字、区县连环、公司后缀连环、
  5 万层括号嵌套等）断言单次脱敏 < 5s，实测最坏 332ms / 100k 字符——防止将来改规则引入正则回溯爆炸。

### 兼容性（CI）

- **真机跨平台矩阵**：CI 新增 macOS 与 Windows 两个 runner 跑同一套测试。此前 Windows 支持只有
  代码审查 + 本地回归，现在是真机证据；Windows 上的 CRLF/路径/venv 布局问题会在 CI 直接暴露。
- **宿主缝巡检进 CI**：新增 `compat` job 跑 `tools/dsh-compat-check.mjs`（0.1.1-rc.2 / 0.1.5-rc.2 /
  0.1.6-alpha.2），上游改事件名/服务名/客户端模块时 CI 报警，而不是等用户装完发现卡片不见了。

### 易用性

- **DSH Desktop 上的状态行不再是「插件状态未知」**：宿主没有 `remote.pluginInventory` 时，
  改为用 `settingsScope` 命名空间的可用性推断状态——命名空间由本插件 host 半边注册，
  就绪即「插件已启用」、不可用即「插件未启用」，只有读取中才显示「未知」（回归见 accuracy
  desktop 场景：就绪 → 已启用，不可用 → 未启用）。
...
- 免责声明改为显式警示块，归入「关于」分区：「免责声明：脱敏为启发式本地处理，
  无法保证零漏检；重要数据请自行评估并保留原文。」不再是正文里的一句裸话；
- 卡片按「开关 / 自定义敏感词 / 使用说明 / 关于」分区排版，分区之间以细线分隔；
  案号、出生日期归入「默认保留项（开启后改为脱敏）」子分组，说明与普通开关相反的语义；
- 「复制状态」与「复制更新命令」合并为同一行操作，减少纵向堆叠；
- accuracy 测试新增「免责声明」文案断言，防止后续布局调整丢失该标签。

### 新功能（M3 文本侧前置：base64 文本预检）

- 新增 `preflightBase64`（默认 `false`）：媒体/文件块携带的 base64 载荷若可判定为
  UTF-8 文本（含 dataURL 形态），先在本地解码、按同一套规则脱敏，再回编码后上云；
  二进制内容（如图片像素）无法判定时仍按 `nonTextPolicy` 处置——`strip` 剥离、
  `block` 拒绝，`allow` 需显式 `allowRawMedia: true`。这是 M3（本地脱敏后再上云）的
  文本侧能力，不是 OCR，不解图片内容。
- 可靠性测试 139 → 157（M7-M21：文本 base64 解码脱敏放行、幂等、嵌套/大写 dataURL 前缀保留、
  同块元数据一并脱敏、白名单、换行空白变体、伪 base64 不判定、无字母数字不判为文本、
  深度超限拦截、官方 attachment 引用块回落策略、超长载荷/元数据不崩不泄）。

### 新功能（离境审计）

- 新增 `egressAudit`（默认 `true`）：每笔发往云端大模型的 llm 请求写一行本地 JSONL
  （`$DSH_HOME/privmask-egress.jsonl`，与 restore 诊断同目录；超过 10MB 轮转为 `.1`），
  记录决策 clean/masked/blocked/error、脱敏字段与类别计数、媒体处置（剥离/预检/原样透传）；
  审计只记决策与计数，不记消息原文，避免审计文件本身成为明文副本；
...
- 新增 `tools/pdf-preflight.mjs`：逐页用 poppler `pdftotext` 提取文本层（`--ocr` 时对空文本页用
  ghostscript 栅格化 + `~/.ocr-tool/ocr.py` 本地 OCR），每页走与对话同一套脱敏规则，
  导出 `<名>.redacted.txt` 脱敏文本预览与 `--report` JSON（页级来源/命中类别），原 PDF 不改写。
  用途是“先核对云端会读到的内容”；对原 PDF 覆写式脱敏仍为规划中的独立能力。
- 可靠性测试 169 → 171（AD1-AD2：文本层脱敏命中/预览无原文、报告页级统计；无 poppler 环境自动跳过）。

### 工具（PDF 覆写式脱敏原型）

- 新增 `tools/redact-pdf.mjs`：pdftotext -bbox 取词框 → 按行走同一套脱敏规则并用占位符映射
  反查敏感片段 → ghostscript 栅格化页面 → 像素级涂黑 → 重组为图片型脱敏 PDF。
  本地 OCR 验证“手机/邮箱被抹除、上下文保留”；依赖 poppler + ghostscript，输出无文本层、
  文件变大，属有损但可外发的覆写方案；保留文本层的原样编辑型 PDF 脱敏仍为长期规划。
- 可靠性测试 171 → 173（AE1-AE2：覆写输出无文本层且涂黑框≥2、页数/尺寸有效；
  无 poppler/gs 环境自动跳过）。
