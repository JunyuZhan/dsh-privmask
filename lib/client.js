window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const DSH_SUPPORT = "dsh 0.1.0-rc.6+（0.1.1-rc.2 / 0.1.5-rc.2 / 0.1.6-alpha.2 已实测核对）";
		...
		function PrivmaskCard(props) {
			...
			const [copiedStatus, setCopiedStatus] = (0, react.useState)(false);
			...
			const active = [field("redactNames") ? "姓名" : "", field("redactCompanies") ? "公司" : "", field("redactOrgs") ? "机关" : "", field("redactAddress") ? "地址" : "", field("redactCredentials") ? "密钥凭据" : ""].filter(Boolean);
			const kept = [!field("redactCaseNumbers") ? "案号" : "", !field("redactDob") ? "出生日期" : "", !field("redactPaths") ? "文件路径" : ""].filter(Boolean);
			const statusText = () => "dsh-privmask v" + PLUGIN_VERSION + " · 生效脱敏：" + (active.join("、") || "无") + "；保留：" + (kept.join("、") || "无");
			return (0, react_jsx_runtime.jsxs)("div", {
				style: row,
				children: [
					...
					writable && cfg !== null ? (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
						...
						(0, react_jsx_runtime.jsx)("div", {
							style: sectionSub,
							children: "默认保留项（开启后改为脱敏）"
						}),
						switchRow("redactCaseNumbers", "案号"),
						switchRow("redactDob", "出生日期"),
						(0, react_jsx_runtime.jsx)("div", {
							style: body,
							children: "当前生效：" + (active.join("、") || "无") + "；保留：" + (kept.join("、") || "无") + "。"
						})
					] }),
						(0, react_jsx_runtime.jsxs)("div", {
							style: section,
					...
					(0, react_jsx_runtime.jsx)("div", {
						...
						children: [(0, react_jsx_runtime.jsx)("button", {
							type: "button",
							style: btnBase,
							onClick: copyStatusCommand,
							children: copiedStatus ? "已复制 ✓" : "复制状态"
						}), (0, react_jsx_runtime.jsx)("button", {
							type: "button",
					...
					(0, react_jsx_runtime.jsx)("div", {
						style: body,
						children: "更新命令复制后在终端运行，并重启 dsh web 生效；复制状态文本可用于审计留档。"
					}),
					(0, react_jsx_runtime.jsx)("div", {
						style: body,
					...
					(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							(0, react_jsx_runtime.jsx)("div", {
								style: body,
								children: "适配宿主：" + DSH_SUPPORT + "；功能随宿主能力自动降级。"
							}),
							(0, react_jsx_runtime.jsx)("div", {
								style: disclaimerBox,
								children: "免责声明：" + DISCLAIMER
							}),
					(0, react_jsx_runtime.jsxs)("div", {
						style: body,
		 ...
		 * remote.pluginInventory 不能写进这份清单：DSH Desktop（desktop profile）的客户端运行时
		 * 没有该服务，硬声明会让整个模块停在 PENDING、卡片永不注册（issue #2）。
		 * 它是可选依赖，改为按能力软注入，拿不到时卡片降级为「插件状态未知」。
		 * settingsScope 同理不能硬声明：0.1.7 起上游把客户端设置 API 换成了 remote.settings，
		 * 现在也走软注入（见 scopeFromRemoteSettings），两代 API 任一可用即可。
		 */
		const inject = [
			"slots",
			"locale"
		...
		function apply(ctx) {
			 ...
			 * web profile 用它显示「插件已启用/未启用」；desktop 等无该服务的宿主保持 undefined，
			 * list() 抛错后卡片显示「插件状态未知」，其余开关走 settingsScope 不受影响。
			 */
			let inventory;
			const inventoryReady = new Promise((resolve) => {
			...
			function scopeFromSettingsScope(svc) {
			...
			function scopeFromRemoteSettings(remote) {
				...
				let snap = { status: "loading", writable: false };
				const refresh = async () => {
					const res = await remote.settings.describe();
					const ns = res && res.ok === true ? (res.value?.namespaces ?? []).find((n) => n.ns === ENTRY) : void 0;
					snap = ns === void 0
						? { status: "unavailable", writable: false }
						: { status: "ready", value: ns.value, revision: ns.revision, writable: res?.value?.writable === true };
				...
				return {
					...
					set: async (field, value) => {
						...
						if (!res || res.ok !== true) throw new Error("settings.update failed: " + String(res?.error?.message ?? "unknown"));
						await refresh();
					}
				};
			}
			/** 按宿主能力选一套设置 API；两代都没有时保持 undefined，卡片仍注册但开关给出明确错误。 */
			let scope;
			const scopeReady = new Promise((resolve) => {
				const timer = setTimeout(resolve, 2000);
				let pending = 2;
				const done = () => {
					if (--pending === 0) {
				...
				const probe = (deps, pick) => {
					try {
						ctx.inject(deps, (sctx) => {
							try {
								const candidate = pick(sctx);
								if (candidate !== void 0 && scope === void 0) scope = candidate;
							} catch {}
							done();
						});
					} catch {
				...
				probe(["settingsScope"], (sctx) => {
					const svc = sctx.settingsScope;
					return svc === void 0 ? void 0 : scopeFromSettingsScope(svc);
				});
				probe(["remote", "remote.settings"], (sctx) => {
					const remote = sctx.remote;
					return remote?.settings === void 0 ? void 0 : scopeFromRemoteSettings(remote);
				});
			});
