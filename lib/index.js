...
export const Config = z.object({
  ...
  /** 地址类脱敏（省市区乡/住址[地址_22]默认开）：当事人隐私核心，起草文书靠入站还原写回真值 */
  redactAddress: z.boolean().default(true),
  /** 事实类脱敏（姓名/案号/出生日期/公司/机关，默认关）：律师工作需真值保证精度，可按需开启 */
  redactFacts: z.boolean().default(false),
...
function createEngine(cfg) {
    ...
    if (rulesCache === null) {
      ...
      if (cfg.cnEntities) {
        for (const rule of CN_RULES) {
          ...
          } else if (rule.cat === 'name' || rule.cat === 'case' || rule.cat === 'dob' || rule.cat === 'company' || rule.cat === 'org') {
            if (cfg.redactFacts) list.push(rule);
          } else {
  ...
  const engine = {
    placeholder, activeRules, redactText, walkBlocks, walkJson, walkTools, sanitizeRequest, reverseMap,
...
export function apply(ctx, config = {}) {
  const cfg = {
    ...
    redactFacts: config.redactFacts ?? false,
  ...
  // 逐块替换按占位符长度降序，避免 EMAIL_1 与 EMAIL_10 部分匹配；
  // text/reasoning 的 delta 尽力还原，block-end 为权威结果（工具参数以块为准）。
  function restoreChunkText(text, rev) {
    if (typeof text !== 'string' || text === '' || rev.size === 0) return text;
    ...
    return (async function* () {
      for await (const chunk of stream) {
        if (chunk.type === 'text-delta' || chunk.type === 'reasoning-delta') {
          yield { ...chunk, text: restoreChunkText(chunk.text, rev) };
        } else if (chunk.type === 'block-end') {
          const block = chunk.block;
          if (block.type === 'text' || block.type === 'reasoning') {
            yield { ...chunk, block: { ...block, text: restoreChunkText(block.text, rev) } };
          } else if (block.type === 'tool-call') {
            yield { ...chunk, block: { ...block, arguments: restoreChunkText(block.arguments, rev) } };
          } else {
            yield chunk;
          }
        } else {
