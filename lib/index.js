...
/** 插件自用：不带 volatile 标记。volatile 字段被宿主解析后是「活引用盒」，不能当普通值用。 */
const PlainConfig = z.object(FIELDS);
...
export const Config = markVolatile(PlainConfig);
...
export function apply(ctx, config = {}) {
  ...
  const unwrapLive = (raw) => Object.fromEntries(
    Object.entries(raw || {}).map(([k, v]) => [k, isLiveRef(v) ? v.get() : v]),
  );
  /** 非活引用的那部分（宿主可能只把部分字段做成盒）作为刷新时的底，避免被默认值覆盖。 */
  const plainBase = unwrapLive(config);
  let cfg;
  try {
    cfg = PlainConfig(unwrapLive(config));
  } catch (error) {
  ...
  function refreshLiveConfig() {
    ...
    let next;
    try {
      next = PlainConfig({ ...plainBase, ...unwrapLive(liveRefs) });
