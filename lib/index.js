...
function createEngine(cfg) {
  ...
  const engine = { per, activeRules, placeholder, redactText, walkBlocks, walkJson, walkTools, sanitizeRequest };
  return engine;
}
...
export function apply(ctx, config = {}) {
  ...
  const stats = { requests: 0, redacted: 0, fields: 0, errors: 0, lastError: null };
  ...
  return ctx.on('llm/stream', (options, next) => {
    ...
    try {
      const projected = engine.sanitizeRequest(options);
      ...
      stats.requests += 1;
      stats.redacted += 1;
      stats.fields += engine.per.fields;
      if (cfg.logRedactions) {
        console.log('[privmask] 已脱敏请求 provider=' + options.provider + ' model=' + options.model + ' 字段=' + engine.per.fields + ' ' + JSON.stringify(Object.fromEntries(engine.per.counts)));
      }
