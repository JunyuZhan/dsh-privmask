...
import { restoreChunkText, splitPlaceholderTail, countPlaceholderMisses, restoreBlocksForDisplay, restoreWireData, restoreRecord, restoreRecords } from './restore.js';
...
export function apply(ctx, config = {}) {
  ...
  function resolveSettings() {
    try {
      if (ctx.settings && typeof ctx.settings.register === 'function') return ctx.settings;
      if (ctx.get) {
        const nonStrict = ctx.get('settings', false);
        if (nonStrict && typeof nonStrict.register === 'function') return nonStrict;
        const strict = ctx.get('settings');
        if (strict && typeof strict.register === 'function') return strict;
      }
    } catch { /* 探测失败不中断 */ }
    return undefined;
  }
  function installSettingsNamespace() {
    ...
    if (!settings) {
      // 服务可能尚未启动（严格 get 在 fiber 未 ACTIVE 时返回 undefined）：惰性重试
      if (cfg.logRedactions && !settingsWarned) {
        settingsWarned = true;
        console.warn('[privmask] settings 服务暂不可用，运行时开关将在服务就绪后注册');
      }
      return;
    }
    try {
      settingsScope = settings.register('privmask', Config, {
        base: cfg,
        applies: 'live',
      });
      ...
      try {
        for await (const chunk of stream) {
          if (chunk.type === 'text-delta' || chunk.type === 'reasoning-delta') {
            ...
            pending = tail;
            misses += countPlaceholderMisses(head);
        ...
        emitStats('restoreMiss', { count: misses });
        if (cfg.logRedactions) {
            console.log('[privmask] 入站还原未命中占位符 ' + misses + ' 处（映射被逐出或模型改写）');
          }
        }
      }
    })();
  ...
  ctx.on('llm/stream', (options, next) => {
    ...
    // 运行时设置惰性注册：settings 服务可能晚于插件加载完成启动
    if (!settingsApplied) installSettingsNamespace();
    ...
    const wrapRestore = (stream, rctx) => {
      ...
      return restoreStream(stream, rev);
    };
    try {
  ...
  function installDisplayRestore() {
    ...
    if (!sc || typeof sc.page !== 'function' || typeof sc.follow !== 'function') {
      if (!displayWarned) {
        ...
        console.warn('[privmask] 展示层还原未安装：sessionController 服务不可用（候选路径: ' + (candidates.map((c) => c.label).join(', ') || '无') + '；控制探针 llm=' + llmProbe + '）');
      }
      return;
    }
