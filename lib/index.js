...
/** 本地 OCR 支持的图片类型（与 dsh-image-text-fallback 保持一致） */
const OCR_EXT = {
  'image/png': 'png',
  'image/jpeg': 'jpg',
  'image/webp': 'webp',
...
/**
 * 本地 OCR 解释器路径（跨平台）：Python venv 在 Windows 下是 `Scripts\python.exe`，
 * POSIX 下是 `bin/python`。写死 bin/python 会让 Windows 上的 localOcr 直接找不到解释器。
 * @param {string} [homeDir] 家目录（默认当前用户）。
 * @param {string} [platform] 平台标识（默认 process.platform，便于测试）。
 */
export function ocrPythonPath(homeDir = homedir(), platform = process.platform) {
  return platform === 'win32'
    ? join(homeDir, '.ocr-tool', 'venv', 'Scripts', 'python.exe')
...
/** 运行时配置字段（单一来源；下面派生「宿主可见的 Config」与「插件自用的纯 schema」）。 */
const FIELDS = {
  /** 总开关 */
  ...
  /** 显式承认风险后才允许非文本原样透传（默认 false：allow 也会拒绝，需先本地 OCR/脱敏） */
  allowRawMedia: z.boolean().default(false),
  /** base64 文本预检（默认关）：媒体/文件块中可判定为 UTF-8 文本的 base64 载荷先解码脱敏再回编码上云；
   *  二进制与不可判定内容仍按 nonTextPolicy 处置（不是 OCR） */
  preflightBase64: z.boolean().default(false),
  ...
  /** 离境审计（默认开）：每笔发往云端大模型的请求写一行本地 JSONL（$DSH_HOME/privmask-egress.jsonl） */
  egressAudit: z.boolean().default(true),
  /** 本地 OCR 兜底（默认关）：把图片附件在本地转成文本后再进脱敏/上云管线；
   *  依赖 ~/.ocr-tool（与 dsh-image-text-fallback 相同的本地视觉服务）；未启用时图片仍按 nonTextPolicy 处置 */
  localOcr: z.boolean().default(false),
  /** 高级/测试：自定义 OCR 命令（argv 数组）；空则使用默认 ~/.ocr-tool/ocr.py 调用 */
  localOcrCommand: z.array(z.string()).default([]),
  /** 每次脱敏打印一行统计日志 */
  logRedactions: z.boolean().default(true),
};

/**
 * 宿主可见的 schema：0.1.7+ 的 SettingsForms **只**把标了 `meta.volatile` 的字段纳入设置表单
 * （`volatileForm()` 对没有 volatile 字段的对象返回 undefined，整个插件条目都不进表单），
 * 并把它们的值换成交互盒（`{ get(), [write] }`）；而 schemastery ≤3.18.2（0.1.5 宿主）
 * 没有 `Schema.prototype.volatile`，直接调用会在加载期抛错——所以按能力派生。
 *
 * 注意：schemastery 的 `Schema(x)` 对已有 schema **返回同一实例**（不是克隆），所以
 * `markVolatile` 会就地改写这些子 schema —— 因此下面用同一个 schema 校验后**解盒**，
 * 而不再另建一份「纯 schema」（那样会被一起改成 volatile，解析出一堆盒子）。
 */
function markVolatile(schema) {
  if (typeof schema.volatile !== 'function') return schema;
  if (!schema.dict) return schema.volatile();
  return z.object(Object.fromEntries(Object.entries(schema.dict).map(([key, child]) => [key, markVolatile(child)])));
}
export const Config = markVolatile(z.object(FIELDS));

/** 活引用盒判定（0.1.7+ 的 schemastery 把 volatile 字段解析成 `{ get(), [write] }`）。 */
const isLiveRef = (v) => typeof v === 'object' && v !== null && typeof v.get === 'function';

/**
 * 校验配置并解盒——**所有内部消费者都应走这里**（`apply` 与各 CLI）。
 * 直接 `Config(raw)` 在 schemastery ≥3.18.4 上会把 volatile 字段解析成活引用盒，
 * 后续 `cfg.xxx.includes(...)` 之类立刻炸（CI 真机与本地都踩到过）。
 */
export function resolveConfig(raw) {
  const unbox = (v) => (isLiveRef(v) ? v.get() : v);
  const input = Object.fromEntries(Object.entries(raw || {}).map(([k, v]) => [k, unbox(v)]));
  return Object.fromEntries(Object.entries(Config(input)).map(([k, v]) => [k, unbox(v)]));
}
...
export function apply(ctx, config = {}) {
  ...
  //
  // 0.1.7+（SettingsForms）把 volatile 字段的值传成「活引用盒」`{ get(), [write] }`：
  // 校验前先拆盒取当前值，同时留着盒子做变更探测（用户改设置后无需重载进程）。
  const isLiveRef = (v) => typeof v === 'object' && v !== null && typeof v.get === 'function';
  const unbox = (v) => (isLiveRef(v) ? v.get() : v);
  const unboxAll = (raw) => Object.fromEntries(Object.entries(raw || {}).map(([k, v]) => [k, unbox(v)]));
  const liveRefs = Object.fromEntries(Object.entries(config || {}).filter(([, v]) => isLiveRef(v)));
  /** 解盒后的入参既用于校验，也留作刷新时的底（宿主可能只把部分字段做成盒）。 */
  const plainBase = unboxAll(config);
  let cfg;
  try {
    cfg = unboxAll(Config(plainBase));
  } catch (error) {
    throw new Error('[privmask] 配置校验失败: ' + (error && error.message ? error.message : error));
  }

  /** 有活引用盒时探测设置是否被改：变了就按新值重建配置与引擎（等价于旧宿主的 watch）。 */
  let liveSignature = JSON.stringify(cfg);
  function refreshLiveConfig() {
    if (Object.keys(liveRefs).length === 0) return;
    let next;
    try {
      next = unboxAll(Config({ ...plainBase, ...unboxAll(liveRefs) }));
    } catch { return; /* 用户填了非法值：保持上一份有效配置，等改正后再生效 */ }
    ...
    if (cfg.logRedactions) {
      console.log('[privmask] 设置已更新（volatile 活引用，live 生效）: enabled=' + cfg.enabled + ' redactNames=' + cfg.redactNames + ' redactCompanies=' + cfg.redactCompanies + ' redactOrgs=' + cfg.redactOrgs + ' redactAddress=' + cfg.redactAddress + ' redactCredentials=' + cfg.redactCredentials);
    }
    emitStats('settingsUpdated', { enabled: cfg.enabled, fields: Object.keys(next).length, source: 'volatile' });
  }
  ...
  let diagTail = Promise.resolve();
  function diag(line) {
    if (cfg.logRedactions) console.log('[privmask] ' + line);
    // 串行追加 + 超限轮转（与 egress 审计同策略）：诊断日志此前无上限，长期运行会无限增长
    const msg = new Date().toISOString() + ' ' + line + '\n';
    diagTail = diagTail.then(async () => {
      try {
        const st = await stat(DIAG_FILE).catch(() => null);
        if (st !== null && st.size > DIAG_MAX) await rename(DIAG_FILE, DIAG_FILE + '.1').catch(() => {});
      } catch { /* 轮转失败不阻塞主链路 */ }
      await appendFile(DIAG_FILE, msg);
    }).catch(() => {});
  }
  diag('启动 cfg: restoreInbound=' + cfg.restoreInbound + ' dropSessionId=' + cfg.dropSessionId + ' persistMapping=' + cfg.persistMapping + ' nonTextPolicy=' + cfg.nonTextPolicy + ' preflightBase64=' + cfg.preflightBase64 + ' egressAudit=' + cfg.egressAudit + ' enabled=' + cfg.enabled);
  ...
  // ── 宿主适配自检：升级后“有没有降级”要一眼可见，而不是静默 ──
  const adaptProbe = (() => {
    const out = { llm: false, settings: false, loaderEntries: null, sessionController: false }
    try {
      out.llm = Boolean(ctx.get && ctx.get('llm'))
      out.settings = Boolean(ctx.get && ctx.get('settings'))
    } catch { /* ignore */ }
    try {
      const loader = (ctx.get && ctx.get('loader')) || ctx.loader
      if (loader && typeof loader.entries === 'function') {
        let entries = 0
        for (const entry of loader.entries()) {
          if (!entry) continue
          entries += 1
          if (entry.id === 'session-controller') out.sessionController = true
        }
        out.loaderEntries = entries
      }
    } catch { /* ignore */ }
    return out
  })()
  const degraded = []
  if (!adaptProbe.sessionController) degraded.push('sessionController（展示层还原不可用，界面用户消息显示占位符）')
  console.log('[privmask] 适配检查: llm=' + (adaptProbe.llm ? '✓' : '✗')
    + ' settings=' + (adaptProbe.settings ? '✓' : '惰性注册（服务就绪后自动启用）')
    + ' 模块表条目=' + String(adaptProbe.loaderEntries)
    + ' sessionController=' + (adaptProbe.sessionController ? '✓' : '✗')
    + (degraded.length ? '；降级项：' + degraded.join('；') : '；全量能力可用'));
  ...
  // ── 本地 OCR 兜底（M3，默认关）：启用 localOcr 时，在请求进入 llm/stream 水瀑前
  // 先把 image 附件转成本地 OCR 文本（与 dsh-image-text-fallback 同构、可叠加）：
  // OCR 文本随后走常规脱敏，图片字节不出本地。依赖 ~/.ocr-tool 本地视觉服务。──
  if (cfg.localOcr) {
    const attachmentsSvc = () => (ctx.get && ctx.get('attachments')) || ctx.attachments;
    const shortError = (error) => String(error && error.message ? error.message : error)
      .replace(/[A-Za-z]:\\[^\s"'*?<>|]*/g, '<路径>') // Windows 盘符路径（C:\Users\...）
      .replace(/\/[^\s:]*/g, '<路径>')
      .slice(0, 200);
    const runOcrFile = async (file, signal) => {
      const cmd = cfg.localOcrCommand.length > 0
        ? cfg.localOcrCommand
        : [ocrPythonPath(), join(homedir(), '.ocr-tool', 'ocr.py')];
      for (const profile of ['balanced', 'accurate']) {
        let out = null;
        try {
          const args = profile === 'balanced'
            ? [...cmd.slice(1), file, '--mode', 'json', '--both']
            : [...cmd.slice(1), file, '--mode', 'json', '--both', '--profile', 'accurate'];
          const { stdout } = await execFileAsync(cmd[0], args, { timeout: 120000, maxBuffer: 64 * 1024 * 1024, signal });
          out = JSON.parse(stdout.trim());
        } catch (error) {
          return { ok: false, error: shortError(error) };
        }
        if (out && out.ok) return { ok: true, text: String(out.result ?? '') };
        if (profile === 'accurate') {
          return { ok: false, error: shortError((out && (out.metadata && out.metadata.error)) || (out && out.error) || 'unknown') };
        }
      }
      return { ok: false, error: 'unknown' };
    };
    const ocrImage = async (attachment, signal, cache) => {
      const key = attachment && attachment.attachmentId;
      if (key !== undefined && key !== null && cache.has(key)) return cache.get(key);
      const fail = (error) => {
        const r = { ok: false, error: shortError(error) };
        if (key !== undefined && key !== null) cache.set(key, r);
        return r;
      };
      const att = attachmentsSvc();
      if (!att || typeof att.readImage !== 'function') {
        return fail('attachments 服务不可用，无法本地 OCR（图片将按 nonTextPolicy 处置）');
      }
      let stored = null;
      try {
        stored = await att.readImage(attachment, signal);
      } catch (error) {
        return fail(error);
      }
      const ext = OCR_EXT[(attachment && attachment.mediaType) || ''] || 'png';
      const dir = await mkdtemp(join(tmpdir(), 'privmask-ocr-'));
      const file = join(dir, 'img-' + randomUUID() + '.' + ext);
      try {
        await writeFile(file, stored && stored.data);
        const r = await runOcrFile(file, signal);
        if (key !== undefined && key !== null) cache.set(key, r);
        return r;
      } catch (error) {
        return fail(error);
    ...
    const visitBlocks = async (blocks, cache, signal, replaced) => {
      let changed = false;
      const out = [];
      for (const block of blocks || []) {
        ...
        if (block.type === 'image' && block.attachment) {
          changed = true;
          replaced.value += 1;
          const r = await ocrImage(block.attachment, signal, cache);
          if (r.ok) {
            out.push({ type: 'text', text: String(r.text) });
          } else if (cfg.nonTextPolicy === 'block') {
            out.push(block); // 保持原块：llm/stream 门禁会整体拒绝
          } else {
            out.push({ type: 'text', text: '(图片本地OCR不可用: ' + r.error + ')' });
          }
          continue;
        }
        if (block.type === 'tool-result' && Array.isArray(block.content)) {
          const inner = await visitBlocks(block.content, cache, signal, replaced);
          if (inner.changed) {
            changed = true;
    ...
    const downgrade = async (options) => {
      ...
      for (const m of options.messages) {
        if (m && Array.isArray(m.content)) {
          const r = await visitBlocks(m.content, cache, options.signal, replaced);
          if (r.changed) {
            changed = true;
            messages.push({ ...m, content: r.blocks });
          } else {
            messages.push(m);
          }
        } else {
      ...
      if (cfg.logRedactions) {
        console.log('[privmask] 本地 OCR 已替换 ' + replaced.value + ' 个图片块（图片字节不出本地）');
      }
      return { ...options, messages };
    };
    const origPrepare = typeof llm.prepareCall === 'function' ? llm.prepareCall.bind(llm) : null;
    if (origPrepare) {
      llm.prepareCall = async (callConfig, signal) => {
        const call = await origPrepare(callConfig, signal);
        const callStream = call && typeof call.stream === 'function' ? call.stream.bind(call) : null;
        if (!callStream) return call;
        return {
          ...call,
          stream: async function* (options) {
            yield* callStream(await downgrade(options));
          },
        };
      };
    }
    const origStreamMethod = typeof llm.stream === 'function' ? llm.stream.bind(llm) : null;
    if (origStreamMethod) {
      llm.stream = async function* (options) {
        yield* origStreamMethod(await downgrade(options));
      };
    }
    const origResolveInfo = typeof llm.resolveModelInfo === 'function' ? llm.resolveModelInfo.bind(llm) : null;
    if (origResolveInfo) {
      llm.resolveModelInfo = async (provider, model, signal) => {
        const info = await origResolveInfo(provider, model, signal);
    ...
    if (cfg.logRedactions) {
      console.log('[privmask] 本地 OCR 兜底已启用：图片附件将在本地转文本后进入脱敏管线（localOcr=true）');
    }
  }
  ...
  // ── 离境审计：每笔发往云端的 llm 请求留痕（本地 JSONL，随 restore 日志同目录；
  // 只记录脱敏决策与媒体处置计数，不记录消息原文，避免审计日志本身成为明文副本）──
  const EGRESS_FILE = join(process.env.DSH_HOME || join(homedir(), '.dsh'), 'privmask-egress.jsonl');
  const EGRESS_MAX = 10 * 1024 * 1024;
  let egressTail = Promise.resolve();
  let egressDirReady = null;
  function auditEgress(entry) {
    if (!cfg.egressAudit) return;
    const line = JSON.stringify({ v: 1, ts: new Date().toISOString(), ...entry }) + '\n';
    egressTail = egressTail.then(async () => {
      if (egressDirReady === null) {
        egressDirReady = mkdir(dirname(EGRESS_FILE), { recursive: true }).catch(() => {});
      }
      await egressDirReady;
      try {
        const st = await stat(EGRESS_FILE).catch(() => null);
        if (st !== null && st.size > EGRESS_MAX) {
          await rename(EGRESS_FILE, EGRESS_FILE + '.1').catch(() => {});
        }
      } catch { /* 旋转失败不阻塞主链路 */ }
      await appendFile(EGRESS_FILE, line);
    }).catch((error) => {
      console.warn('[privmask] 出站审计写入失败: ' + (error && error.message ? error.message : error));
    });
  }
  const mediaOf = (rctx) => ({
    dropped: rctx ? (rctx.mediaDropped || 0) : 0,
    preflight: rctx ? (rctx.mediaPreflight || 0) : 0,
    raw: rctx ? (rctx.mediaRaw || 0) : 0,
  });
  ...
  function installSettingsNamespace() {
    ...
    ctx.inject(['settings'], (sctx) => {
      ...
      try {
        ...
        // 上游改过两代设置 API：0.1.1–0.1.5 是「插件注册命名空间 + watch」；
        // 0.1.7 起换成 SettingsForms（插件声明自动页策略，写回 profile 后由 loader 重新 apply）。
        if (typeof settings.register !== 'function') {
          if (typeof settings.configure !== 'function') {
            throw new Error('settings 服务既无 register 也无 configure（形态未知，可能又变了）');
          }
          settings.configure({ auto: true });
          settingsApplied = true;
          if (cfg.logRedactions) {
            console.log('[privmask] 运行时设置已注册（0.1.7+ SettingsForms：自动页已声明，配置写回 profile 后由 loader 重载生效）');
          }
          return;
        }
        ...
        settingsScope.watch((next) => {
          ...
          // 校验后解盒：0.1.7+ 的 schemastery 会把 volatile 字段解析成活引用盒，不能直接当值用
          const resolved = unboxAll(Config(next));
          cfg = resolved;
          engine = createEngine(cfg);
          if (cfg.logRedactions) {
  ...
  try { installSettingsNamespace(); } catch (error) {
    console.warn('[privmask] 设置命名空间安装失败（保持配置文件模式，不影响脱敏主链路）', error);
  }
  ...
  ctx.on('llm/stream', (options, next) => {
    refreshLiveConfig(); // 0.1.7 的 volatile 活引用：用户改设置后下一次请求即生效
    ...
    try {
      ...
      if (result !== null && typeof result === 'object' && result.blocked !== undefined) {
        ...
        auditEgress({ provider: options.provider, model: options.model, decision: 'blocked', reason: result.blocked });
        return blockedStream(result.blocked);
      }
      const projected = result;
      if (projected === options) {
        stats.requests += 1;
        const media = mediaOf(rctx);
        auditEgress({ provider: options.provider, model: options.model, decision: 'clean', media, rawMedia: media.raw > 0 });
      ...
      auditEgress({
        ...
        decision: 'masked',
        fields: rctx.fields,
        counts: Object.fromEntries(rctx.counts),
        media,
        rawMedia: media.raw > 0,
      });
      ...
      stats.requests += 1;
      auditEgress({ provider: options.provider, model: options.model, decision: 'error', reason: String(stats.lastError || '').slice(0, 500) });
  ...
  try { installDisplayRestore(); } catch (error) {
    console.warn('[privmask] 展示层还原安装失败（已降级为占位符显示；不影响脱敏主链路）', error);
  }
}
