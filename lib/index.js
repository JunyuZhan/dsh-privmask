...
export function apply(ctx, config = {}) {
  ...
  function installSettingsNamespace() {
    ...
    let settings;
    try { settings = ctx.settings || (ctx.get && ctx.get('settings')); } catch { settings = undefined; }
  ...
  ctx.on('llm/stream', (options, next) => {
    ...
    const wrapRestore = (stream, rctx) => {
      ...
      if (rev.size === 0) {
        if (cfg.logRedactions) {
          console.log('[privmask] 入站还原跳过：会话映射为空 sessionId=' + String(options.sessionId) + '（占位符将保留在回复中）');
        }
        return stream;
      }
      if (cfg.logRedactions) {
        console.log('[privmask] 入站还原包装 rev=' + rev.size + ' sessionId=' + String(options.sessionId));
      }
