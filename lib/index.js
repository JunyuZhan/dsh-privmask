...
export function apply(ctx, config = {}) {
  ...
  function installDisplayRestore() {
    ...
    // session-controller 可能注册在 web-app 内层 include：通过 loader 树枚举所有 entry，
    ...
    try {
      ...
      if (loader && typeof loader.entries === 'function') {
        loaderProbe = { present: true, entries: 0, sessionSeen: false, sessionFiber: null, loaders: 1 };
        const visited = new Set();
        const walk = (ld, depth) => {
          if (!ld || depth > 6 || visited.has(ld)) return;
          visited.add(ld);
          loaderProbe.loaders += depth === 0 ? 0 : 1;
          let iter;
          try { iter = ld.entries(); } catch { return; }
          for (const entry of iter) {
            if (!entry) continue;
            loaderProbe.entries += 1;
            if (entry.id === 'session-controller') {
            ...
            const fc = entry.fiber && entry.fiber.ctx;
            if (fc && fc !== ctx) walk(loaderOf(fc), depth + 1);
          }
        };
        walk(loader, 0);
      }
