...
import { Config, resolveConfig } from './index.js'
...
function accumulateDelta(counts, before, rctx) {
  for (const [cat, n] of rctx.counts) {
    const prev = before.get(cat) || 0
    if (n > prev) counts[cat] = (counts[cat] || 0) + (n - prev)
  }
}

/** 把一段 <w:p> 内所有 w:t 文本按顺序合并后整体脱敏，结果写入第一个 w:t，
 *  其余 w:t 置空（保留 run 数量与结构；跨 run 样式差异会被合并到首 run，需抽样核对）。
 *  返回 { changed, xml }。 */
function redactParagraphWhole(pXml, engine, rctx, counts) {
  const parts = []
  let changedAny = false
  const next = pXml.replace(TEXT_NODE_RE, (whole, inner) => {
    parts.push(decodeXml(inner))
    return whole
  })
  const merged = parts.join('')
  if (merged === '') return { changed: false, xml: pXml }
  const before = new Map(rctx.counts)
  const r = engine.redactText(merged, rctx)
  if (!r.changed) return { changed: false, xml: pXml }
  accumulateDelta(counts, before, rctx)
  // 把脱敏结果整体放入第一个 w:t；其余 w:t 清空并保留标签结构
  let first = true
  const out = next.replace(TEXT_NODE_RE, (whole) => {
    if (first) {
      first = false
      changedAny = true
      return whole.replace(/>[\s\S]*?<\/w:t>/, '>' + encodeXml(r.text) + '</w:t>')
    }
    return whole.replace(/>[\s\S]*?<\/w:t>/, '></w:t>')
  })
  return { changed: changedAny, xml: out }
}
 ...
 * @param {object} [opts] - { wholeParagraph?: boolean } 整段合并识别（默认 false）
 */
export function redactDocx(input, cfg = {}, opts = {}) {
  const resolved = resolveConfig(cfg)
  const engine = createEngine(resolved)
  const entries = parseZip(input)
  const counts = {}
  let nodes = 0
  let wholeParagraphs = 0
  ...
  for (const entry of entries) {
    ...
    nodes += (xml.match(TEXT_NODE_RE) || []).length
    let changed = false
    if (opts.wholeParagraph) {
      xml = xml.replace(PARAGRAPH_RE, (p) => {
        const rr = redactParagraphWhole(p, engine, rctx, counts)
        if (rr.changed) { changed = true; wholeParagraphs += 1 }
        return rr.xml
      })
    } else {
    ...
    if (changed && opts.wholeParagraph) entry.data = Buffer.from(xml, 'utf8')
  }
  return { buffer: writeZip(entries), stats: { nodes, wholeParagraphs, counts, files: entries.length } }
}
