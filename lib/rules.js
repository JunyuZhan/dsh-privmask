...
export const RULES = [
  ...
  // mailto 整地址优先全量脱敏：避免被下方 userinfo 规则拆成「mailto:user:[KEY]@domain」
  // 导致邮箱域名残留
  { cat: 'email', re: /mailto:[^@\s]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/gi },
  // URL / 连接串 user:password@ 用户信息：整段密码脱敏（特殊字符密码此前会被邮箱规则部分漏出）
  // scheme 起点加负向后顾：避免在无 "://" 的长文本上每个字母都作起点重扫（O(n²)）；
  // 密码允许包含原始 '@'（畸形但真实存在），吞到主机名前的最后一个 @，并做主机形态前瞻，
  // 避免把 URL 路径里普通 @ 误当凭据；密码字符集排除方括号保证占位符不会被二次包裹
  { cat: 'key', re: /(?<![A-Za-z0-9+.-])([a-z][a-z0-9+.-]*:\/\/)[^/\s:@]+:([^/\s:\[\]\\]{3,})@(?=\S)/gi, secretGroup: 2 },
  // 冒号前导排除 mailto:/tel: 等：这些是收件人/号码而非 userinfo；反斜杠排除 Windows 路径
  { cat: 'key', re: /(?<![A-Za-z0-9._%+\-/:\\])([A-Za-z0-9._%+-]+):([^/\s:\[\]\\]{3,})@(?=\S)/gi, secretGroup: 2 },
  // 起始加本地部分字符集的负向后顾：长串无 '@' 时只在合法起点尝试匹配，
  // 避免 `[A-Za-z0-9._%+-]+` 在每次扫描位置都吃到串尾再回溯（O(n²) 卡死）。
  { cat: 'email', re: /(?<![A-Za-z0-9._%+-])[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/g },
...
/** 复姓（首字不在单姓表中的也纳入，如 司马/诸葛/上官/慕容…） */
export const COMPOUND_SURNAMES = ['司马', '诸葛', '上官', '欧阳', '夏侯', '皇甫', '东方', '独孤', '南宫', '慕容', '司徒', '司空', '尉迟', '公孙', '长孙', '宇文', '令狐', '端木', '申屠', '澹台', '拓跋', '赫连', '呼延', '完颜', '富察', '爱新觉罗'];
/** 复姓首字集合：快速判定候选是否可能以复姓开头 */
export const COMPOUND_START = new Set(COMPOUND_SURNAMES.map((s) => s[0]));
/** 单遍切块替换：spans 需按 start 升序，成员 { start, len, placeholder } */
function replaceFound(out, spans) {
  if (spans.length === 0) return out;
  const parts = [];
  let last = 0;
  for (const f of spans) {
    parts.push(out.slice(last, f.start), f.placeholder);
    last = f.start + f.len;
  }
  parts.push(out.slice(last));
  return parts.join('');
}
/** 单个汉字判定（与 HAN 同范围），避免在 pickName 前空跑整轮尾词剥离 */
const hanChRe = new RegExp('^[' + HAN + ']$');
...
export const CN_RULES = [
  ...
  // 带「统一社会信用代码/USCC/营业执照号」等明确标注的 18 位编码始终脱敏：
  // 作者已明确标注，不依赖校验位，且大小写字母均可（修复「统一社会信用代码 …67x」小写漏出）；
  // 裸「编号/订单号」等泛化标签不作触发词（合法 USCC 仍会被无上下文严格规则命中）
  { cat: 'uscc', re: /(统一社会信用代码|社会信用代码|信用代码|营业执照号|营业执照注册号|USCC|uscc)\s*[:：]?\s*([0-9A-Za-z]{18})(?![0-9A-Za-z])/g, secretGroup: 2 },
  ...
  // 收尾字符类要排除 \r：Windows 的 CRLF 文本里行尾的 \r 会被吞进地址 span，
  // 替换后整行行尾从 \r\n 变成 \n（跨平台保真回归见 reliability AG1）
  { cat: 'addr', re: new RegExp('(' + ADDR_CTX.join('|') + ')\\s*[:：]?\\s*([^\\n\\r。；;，,\\[]{4,60})', 'g'), secretGroup: 2 },
  // 「住所：」是法律文书高频地址前缀（不在 ADDR_CTX，避免「住所证明」等无冒号场景误伤）
  { cat: 'addr', re: /住所\s*[:：]\s*([^\n\r。；;，,\[]{4,60})/g, secretGroup: 1 },
  // 无冒号的「住所」仅在紧跟地域词（省/市/区/县…）时才视为地址前缀：
  // 「住所贵州省毕节市…」可脱敏，而「住所证明文件/变更登记/无固定场所」等短语不触发
  { cat: 'addr', re: /住所(?=[^，,。；;\n]{0,8}?(?:省|市|自治州|地区|盟|区|县))([^\n\r。；;，,\[]{4,60})/g, secretGroup: 1 },
  {
    cat: 'org', fn: (text, engine, rctx) => {
      ...
      while ((m = keyRe.exec(out)) !== null) {
        ...
        prefix = trimmed;
        // 「新开了一家律师事务所」「这家医院」「那家银行」等数量/动作引导是泛指，不是具体机构
        if (/^(新开|新设|开了一?家|一家|这家|那家|有家|好几家|首家|第二家|第三家)/.test(prefix)) continue;
        ...
        // 法院/法庭是国家审判机关名：公开信息，不指向自然人，且案号里的法院代字本就写明法院
        // （脱了法院名却留案号等于白脱）。默认保留，便于管辖判断与「此致 XX 法院」起草。
        // 需要遮罩某个具体法院时，把它的全称加进 customTerms 即可（自定义词表优先于本规则）。
        if (/法院$|法庭$/.test(spanText)) continue;
  ...
  // 镇乡村结尾的地址链：后不能跟道路字（避免「中关村大街」被截成「中关村」）
  {
    cat: 'addrchain', fn: (text, engine, rctx) => {
      ...
      out = replaceFound(out, found);
      // 续扫：链尾占位符后紧跟的道路（如「粤海街道科技园路」）在第二轮会被左边界挡掉，
      // 这里用「紧跟占位符 ]」的限定续扫补遮，保证地址链不残留道路名
      const afterRe = new RegExp('(?<=\\])[' + HAN + ']{2,8}(?:大道|路|街|巷)[' + HAN + '\\d]{0,12}号?(?![0-9])', 'g');
      const extra = [];
      afterRe.lastIndex = 0;
      let m2;
      while ((m2 = afterRe.exec(out)) !== null) {
        const span = m2[0];
        if (/门口|楼下|屋里|家里|村里|园区/.test(span) || engine.isPreserved(span)) {
          afterRe.lastIndex = m2.index + m2[0].length;
          continue;
        }
        extra.push({ start: m2.index, len: span.length, text: span });
        afterRe.lastIndex = m2.index + m2[0].length;
      }
      if (extra.length > 0) {
        for (const f of extra) f.placeholder = engine.placeholder('street', f.text, rctx);
        out = replaceFound(out, extra);
      }
  ...
  { cat: 'street', re: new RegExp('(?<![' + HAN + '\\]])[' + HAN + ']{2,8}(?:区|街道|路|大道|街|巷)[' + HAN + '\\d]{0,12}号?(?![0-9])', 'g'), validator: (s) => {
    // 法院/法庭默认保留（见 org 规则）：不在这里放行的话会以「X区人民法院」形态被当路名吞掉
    if (/法院$|法庭$/.test(s)) return false;
    ...
    // 「南山区政务服务中心搬迁」等服务/机构地点不是街道名（按 区 结尾误匹配）
    if (/(政务|社区|文体|党群|养老|托育|公共|就业|便民)(服务)?中心/.test(s)) return false;
    // 「在小区门口等我」「家门口的路」「翠湖小区门口」：门口/楼栋/屋里等生活场景不是街道名
    if (/门口|楼下|屋里|家里|村里/.test(s)) return false;
    // 「前方路口左转」「十字路口红绿灯」「三岔路口有家便利店」：泛指路口不是街道名
    if (/路口/.test(s) && /^(前方|这个|那个|十字|三岔|丁字|下个|别的|一个|有|去|在|注意|走到|路过)/.test(s)) return false;
  ...
  } },
  {
    cat: 'name', fn: (text, engine, rctx) => {
      ...
      function pickName(rest) {
        // 前置约束：姓名首字必须是汉字且为单姓或复姓首字；
        // 不符合时（动词/非姓氏开头）无需跑 12 字×尾词剥离的完整尝试
        const ch0 = rest[0];
        if (!ch0 || !hanChRe.test(ch0)) return null;
        if (!SURNAMES.includes(ch0) && !COMPOUND_START.has(ch0)) return null;
        ...
        for (let len = max.length; len >= 2; len--) {
          ...
          const first = name[0];
          if (!SURNAMES.includes(first) && !COMPOUND_SURNAMES.some((s) => name.startsWith(s))) continue;
      ...
      while ((m = ctxRe.exec(out)) !== null) {
        ...
        consumedUntil = idx;
        // 线性推进：跳到本组已消费的末尾，避免对长文档反复从「当前位置+1」重扫尾部（O(n²)）
        ctxRe.lastIndex = Math.max(m.index + m[0].length, consumedUntil);
      }
      // 白名单：命中的值即使符合规则也保留原文
      for (let i = found.length - 1; i >= 0; i--) {
        if (engine.isPreserved(found[i].name)) found.splice(i, 1);
      }
      // 上下文扫描产出的区间按文档顺序递增；后续“已知姓名补扫”需要反复判覆盖，
      // 用二分代替线性 some()，避免命中数×已发现数 的 O(n²)
      const intervals = found.map((f) => ({ start: f.start, end: f.start + f.name.length }));
      function isCovered(pos) {
        ...
        while (lo <= hi) {
          const mid = (lo + hi) >> 1;
          if (intervals[mid].start <= pos) { ans = mid; lo = mid + 1; }
          else hi = mid - 1;
        }
        return ans >= 0 && pos < intervals[ans].end;
      }
      ...
      // 按文档顺序排序编号，保证占位符序号与出现顺序一致；单遍切块替换避免逐条重建
      found.sort((a, b) => a.start - b.start);
      for (const f of found) f.placeholder = engine.placeholder('name', f.name, rctx);
      if (found.length > 0) {
        // 单遍切块替换：逐条 slice+concat 重建整串在替换数多时是 O(替换数×文本长)
        const parts = [];
        let last = 0;
        for (const f of found) {
          parts.push(out.slice(last, f.start), f.placeholder);
          last = f.start + f.name.length;
        }
        parts.push(out.slice(last));
        out = parts.join('');
        changed = true;
  ...
  },
  {
    cat: 'company', fn: (text, engine, rctx) => {
      ...
      while ((m = keyRe.exec(out)) !== null) {
        ...
        // 「新开了一家律师事务所」「这家公司」等数量/动作引导是泛指，不是具体机构
        if (/^(新开|新设|开了一?家|一家|这家|那家|有家|好几家|首家|第二家|第三家)/.test(trimmed)) continue;
        // 无专名的「社区/政务/文体等（服务）中心」是泛指服务点，不是公司/机构名
        // （有地名前缀如「南山区政务服务中心」因无法匹配 ^ 仍正常遮罩）
        if (/^(社区|政务|文体|党群|养老|托育|公共|就业|便民)(服务)?中心$/.test(trimmed)) continue;
        // company 规则按 ORG_KEYS 拆词：key 为「中心」时前缀为「社区服务」等
        if (key === '中心' && /^(社区|政务|文体|党群|养老|托育|公共|就业|便民)(服务)?$/.test(trimmed)) continue;
    ...
    list.push({
      cat: 'custom', fn: (text, engine, rctx) => {
        ...
        for (const term of cfg.customTerms) {
          ...
          let idx = 0;
          let i;
          while ((i = out.indexOf(term, idx)) >= 0) {
            // 精确子串匹配：词表由用户维护，命中即脱敏（含于长词时同样命中）
            if (!engine.isPreserved(term)) spans.push({ start: i, len: term.length, text: term });
            idx = i + term.length;
          }
          if (spans.length > 0) {
            for (const f of spans) f.placeholder = engine.placeholder('custom', f.text, rctx);
            out = replaceFound(out, spans);
            changed = true;
          }
        }
        return { changed, text: out };
