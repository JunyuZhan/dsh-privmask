...
/** 递归还原 JSON 形态数据中的占位符（字符串/数组/普通对象；非普通对象原样返回） */
export function restoreJson(value, entries) {
  if (typeof value === 'string') return restoreChunkText(value, entries)
  if (Array.isArray(value)) {
    let changed = false
    const out = value.map((v) => {
      const r = restoreJson(v, entries)
      if (r !== v) changed = true
      return r
    })
  ...
  if (value !== null && typeof value === 'object') {
    ...
    for (const k of Object.keys(value)) {
      const r = restoreJson(value[k], entries)
